...
let chatServer = {
  ...
  chatchPost: function(req) {
    console.log(chatSocket.id + '!!! from ');
    chatIo.to(req.body.id).emit('front chat message', 'Post Request： ' + req.body.msg, req.body.id, USER_TYPE_AGENT);
    chatSocket.broadcast.to(req.body.id).emit('front chat message',
       chatSocket.id + ' ： ' + chatSocket.handshake.query['userType'], req.body.id, chatSocket.handshake.query['userType']);
    console.log(chatSocket.id + '!!! to ');
    }
}

// 【テスト用】http request handler
function handleHttpRequest(app, socket) {
  ...
  //   chatIo.to(req.body.id).emit('front chat message', 'Post Request： ' + req.body.msg, req.body.id, USER_TYPE_AGENT);
  //   socket.broadcast.to(req.body.id).emit('front chat message',
  //     socket.id + ' ： ' + socket.handshake.query['userType'], req.body.id, socket.handshake.query['userType']);
  //   res.render('post');
  // });
