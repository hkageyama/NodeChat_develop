...
let access_db = {
    ...
    app.get('/main', function(req, res) {
      pool.getConnection(function(err, connection) {
        ...
        connection.query(sql, function(error, results, fields) {
          if (error) throw error;
          for (let i = 0; i < results.length; i++) {
            objs.push({'roomId': results[i].room_id});
          }
          connection.release();
          res.render('main', {lists: objs});
        });
      });
    });
  },
  ...
  pool.getConnection(function(err, connection) {
    connection.query(sql, param, function(error, results, fields){
        ...
        socket.join(roomId);
        io.to(roomId).emit('front open form agent', roomId);
        io.to(roomId).emit('front refresh', roomId, socket.id, userType);
        io.to(roomId).emit('front chat message', userType, 'Romm #' + roomId + 'に入室しました。', roomId);
      });
      connection.release();
    });
  },
